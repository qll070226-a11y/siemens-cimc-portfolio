...
uint8_t ota_uart_request_bootloader_upgrade(void)
{
    ...
    return flash_program_bytes(BL_PARAM_PAGE_ADDR, s_param_page_cache, BL_PARAM_PAGE_SIZE) ? 1U : 0U;
}

/* 把当前 APP 的设备 ID 与波特率码同步到 BootLoader 参数页。BL 复位后会按这里设定的
 * 波特率打开 USART1、按设备 ID 收发：
 *   - 否则 M-01 把上位机切到 115200 之后，BL 仍按默认 19200 启动会丢掉所有 0x0502/0x0503；
 *   - 否则 L-01 把 ID 改成非 0x0001 之后，BL 仍按 0x0001 过滤会丢弃升级帧，N-02/N-03 不得分。 */
