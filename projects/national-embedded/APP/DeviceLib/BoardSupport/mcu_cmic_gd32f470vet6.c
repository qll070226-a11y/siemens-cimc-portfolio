...
uint16_t adc_value[ADC_SCAN_NUM];
...
void bsp_rs485_set_baud(uint32_t baud)
{
    usart_disable(USART1);
...
void bsp_adc_init(void)
{
    ...
    gpio_mode_set(ADC1_PORT, GPIO_MODE_ANALOG, GPIO_PUPD_NONE, ADC1_PIN | ADC_CH1RB_PIN | ADC_VREF_PIN);
    ...
    dma_single_data_parameter.number = ADC_SCAN_NUM;
    ...
    adc_channel_length_config(ADC0, ADC_ROUTINE_CHANNEL, ADC_SCAN_NUM);
    
    adc_routine_channel_config(ADC0, ADC_IDX_CH0,  ADC_CHANNEL_10, ADC_SAMPLETIME_15);  
    adc_routine_channel_config(ADC0, ADC_IDX_CH1,  ADC_CHANNEL_11, ADC_SAMPLETIME_15);  
    adc_routine_channel_config(ADC0, ADC_IDX_VREF, ADC_CHANNEL_12, ADC_SAMPLETIME_15);  
    /* ADC trigger config */
...
void bsp_dac_set(uint16_t value)
{
